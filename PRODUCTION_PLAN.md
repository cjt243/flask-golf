...
| Phase 4 — Testing | **Complete** | 2026-02-17 | pytest suite with 78 tests: conftest.py fixtures (fresh DB per test, auth/admin clients, CSRF helper), test_utils (format_score, parse_score_to_int, _api_int, validate_*), test_auth (login_required, admin_required, csrf_required, session expiry, logout), test_picks (submission validation, duplicates, golfer checks), test_leaderboard (scoring, 999 penalty, ranking, ties, owner names, player standings), test_admin (access control, tournament CRUD, user approval/rejection, registration toggle) |
...
| Phase 6 — Merge & Deploy | **Complete** | 2026-02-17 | Merged feature/turso-migration-magic-auth to main (PR #13), verified DO deployment, fixed health check behind HTTPS redirect, rewrote README |
| Post-deploy — UI/UX Production Review | **Complete** | 2026-02-23 | PR #14: Mobile header wrapping, leaderboard empty-state CTA, Choices.js dropdown scroll/containment fix, golfer OWGR ranks in pick dropdowns, registration-closed banner on login, player standings mobile card stacking, footer year fallback cleanup, zero-teams display |
